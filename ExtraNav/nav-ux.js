...
<div class="hb-shell">
    ...
    (function initHoverIntent() {
        ...
        // PATCH: Add/Update CSS for stacking context
        const styleId = "navux-menu-stacking-style";
        if (!document.getElementById(styleId)) {
            const style = document.createElement('style');
            style.id = styleId;
            style.textContent = `
      .has-submenu, .sublist, .panel {
        z-index: 2147483647 !important;
        background: #fff !important;
        opacity: 1 !important;
        box-shadow: 0 2px 18px rgba(0,0,0,0.14);
      }
      .has-submenu.open > .sublist, .has-submenu.open > [role="menu"], .has-submenu.open > .submenu {
        display: block !important;
        z-index: 2147483647 !important;
      }
      .sublist, [role="menu"], .submenu {
        background: #fff !important;
        border-radius: 7px;
        box-shadow: 0 2px 18px rgba(0,0,0,0.14);
        opacity: 1 !important;
    ...
    `;
        ...
        // PATCH: Helper to close all sibling submenus except the current one
        function closeSiblingSubmenus(currentItem) {
            const parent = currentItem.parentElement;
            if (!parent) return;
            Array.from(parent.children).forEach((sibling) => {
                if (sibling !== currentItem && sibling.classList.contains('has-submenu')) {
        ...
        (function initNested() {
            ...
            // ... existing timers/ensureTimers/parseDelay/delays/focusablesSel/getFocusables/first/last/getTrigger ...
            ...
            t.open = setTimeout(() => {
                // PATCH: Close sibling submenus before opening this one
            ...
            item.addEventListener('pointerenter', () => {
                ...
                // PATCH: Close sibling submenus before opening
                closeSiblingSubmenus(item);
                open(item);
            });
            ...
            item.addEventListener('click', (e) => {
                ...
                closeAllExceptFamily(items, item, close);
                closeSiblingSubmenus(item);
                open(item, true); e.preventDefault();
                }
            });
...
(() => {
    ...
    const ICONS = {
        claim: {
            on: svg`<svg width="24" height="24" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M21.541 17.541C22 16.6386 22 15.278 22 13V11C22 8.17157 22 6.75736 21.1213 5.87868C20.2426 5 18.8284 5 16 5H9M18.9305 18.9305C18.168 19 17.2143 19 16 19H8C5.17157 19 3.75736 19 2.87868 18.1213C2 17.2426 2 15.8284 2 13V11C2 8.17157 2 6.75736 2.87868 5.87868C3.38012 5.37724 4.05597 5.16196 5.06953 5.06953" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        sexy: {
            on: svg`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<path d="M3 3L21 21" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        acct: {
            on: svg`<svg xmlns="http://www.w3.org/2000/svg" viewBox="1 0 36 36" fill="none" width="48" height="48"
...
<path d="M3 3L21 21" stroke="#000000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        product: {
            on: svg`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 29" fill="none" width="34" height="34"
